...
class KtvAction extends UserAction{
	...
	public function infos(){
		...
		if(empty($check)){$this->error('请填写好KTV管理页面再进行设置',U($this->type.'/index',array('token'=>$this->token)));}
	...
	//订单设置
	 public function setinfo(){ 
		...
		if(empty($checkdata)){$this->error('请填写好KTV管理页面在再进行设置',U($this->type.'/index',array('token'=>$this->token)));}
		$_POST['token'] = $this->token;
		...
		if(empty($list[0][name])){
			$list[0][name]="选择房间标准";
			$list[0][value]="双人房|小房|中房|大房|标准房|豪华房|总统房";
		}
		//print_r($list);die;
		$this->assign('list',$list);
